  ...
  if (attrList && attrList.length) {
    ...
    for (let i = 0; i < attrList.length; ++i) {
      ...
      if (name && !symbol && !node.hasAttribute(name)) {
        ...
        return false
      } else if (symbol === '|=' && node.getAttribute(name) !== value && !node.getAttribute(name).startsWith(value + '-')) {
        return false
      } else if (symbol === '^=' && !node.getAttribute(name).startsWith(value)) {
        // 属性值以value为开头
        return false
      } else if (symbol === '$=' && !node.getAttribute(name).endsWith(value)) {
        // 属性值以value为结尾
        return false
      } else if (symbol === '*=' && node.getAttribute(name).indexOf(value) === -1) {
        // 属性值包含value
        return false 
      }
    }
